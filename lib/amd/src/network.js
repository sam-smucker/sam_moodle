...
function(
        ...
        if (alertModal !== null) {
            ...
            alertModal.show().then(function() {
                if (typeof redirectUrl === 'string') {
                    alertModal.getRoot().on(ModalEvents.hidden, function() {
    ...
    var keepalive = async function(freq, timeout, identifier, component, redirect = null) {
        ...
        var strings = await Str.get_strings([
            ...
            {key: 'exitactivity', component: 'mod_scorm'},
        ]);
        ...
        // The save button is renamed to "Exit" and will redirect
        // the user when clicked.
        alertModal.getRoot().on(ModalEvents.save, function() {
            window.location.replace(redirectUrl);
        });
